...
async function addColumn({ count = 1 }: { count?: number }) {
  try {
    if (count < 1) {
...
async function removeColumn({ columnId }: { columnId: string }) {
  try {
    const workbook = await ensureWorkbook();
    const { sheetId, sheet } = getActiveSheetSnapshot();
...
async function addRow({ count = 1 }: { count?: number }) {
  try {
    if (count < 1) {
      throw new Error("Count must be at least 1.");
    }
...
async function removeRow({ rowId }: { rowId: string | number }) {
  try {
...
async function readCell({ address }: { address: string }) {
  try {
...
async function readRange({ range, mode }: {
  range: string;
...
async function clearRange({ range }: { range: string }) {
  try {
    const workbook = await ensureWorkbook();
...
async function listFormulas({ category, page: pageProp, pageSize: pageSizeProp }: { category?: string; page?: number; pageSize?: number } = {}) {
  try {
